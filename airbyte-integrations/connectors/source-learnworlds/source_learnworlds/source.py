...
class LearnworldsStream(HttpStream, ABC):
    ...
    `class LearnworldsStream(HttpStream, ABC)` which is the current class
    `class Customers(LearnworldsStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(LearnworldsStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalLearnworldsStream((LearnworldsStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class SourceLearnworlds(AbstractSource):
    def check_connection(self, logger, config) -> Tuple[bool, any]:
        ...
        """
        logger.info("Checking LearnWorlds connection...")
        client_id = config["client_id"]
        client_secret = config["client_secret"]
        response = requests.get(
            f"{config['api_url']}/admin/api/oauth2/access_token",
        ...
        if 'success' not in json_response:
            ...
            logger.info(f"LearnWorlds connection success")
            return True, None
